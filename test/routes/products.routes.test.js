...
describe("Test Routes Products", async function (){
    ...
    it('GET /api/products/  All products', async function () {
        const response = await request.get('/api/products/').set('Cookie', `${AdminCookie.name}=${AdminCookie.value}`);//pasamos la cookie con la autenticacion;
        //console.log(response);
    ...
    it('GET /api/products/mockingproducts/  Mock products', async function () {
        const response = await request.get('/api/products/mockingproducts');//pasamos la cookie con la autenticacion;
        //console.log(response);
        expect(response).to.be.ok;
        expect(response.statusCode).to.be.eql(200);
    });
